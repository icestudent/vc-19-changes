...
#define FOPEN_MAX       20
#define _SYS_OPEN       20
#define TMP_MAX         2147483647 // INT_MAX
#if __STDC_WANT_SECURE_LIB__
...
// CRT_REFACTOR TODO This is required in <fstream> so that the C++ Standard
// Library can get direct access to the stdio stream buffer.  Ideally, we
// should break this dependency.  It may not be possible to do so.  Further
...
__DEFINE_CPP_OVERLOAD_STANDARD_FUNC_0_0(
    char*, __RETURN_POLICY_SAME, _ACRTIMP, gets,
...
_ACRTIMP int __cdecl setvbuf(
    _Inout_                      FILE*  _Stream,
    _Inout_updates_opt_z_(_Size) char*  _Buffer,
    _In_                         int    _Mode,
    _In_                         size_t _Size
    );
...
__DEFINE_CPP_OVERLOAD_STANDARD_NFUNC_0_2_ARGLIST_EX(
    ...
    _Out_writes_(_BufferCount) _Post_maybez_, char,        _Buffer,
    _In_                                      size_t,      _BufferCount,
    _In_z_ _Printf_format_string_             char const*, _Format
    )
