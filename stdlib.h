...
// (also defined in ctype.h).
    ...
    #else
        _ACRTIMP int* __cdecl __p___mb_cur_max(void);
...
_ACRTIMP int __cdecl mbtowc(
    ...
    _In_reads_bytes_opt_(_SrcSizeInBytes) _Pre_opt_z_ char const* _SrcCh,
...
__DEFINE_CPP_OVERLOAD_STANDARD_NFUNC_0_3_SIZE_EX(
    ...
    _Out_writes_z_(_MaxCount), char,           _Dest,
    _In_z_                     wchar_t const*, _Source,
    _In_                       size_t,         _MaxCount,
    _In_opt_                   _locale_t,      _Locale
    )
...
_ACRTIMP errno_t __cdecl _makepath_s(
    _Out_writes_z_(_SizeInWords) char*       _PathResult,
    _In_                         size_t      _SizeInWords,
...
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    // CRT_REFACTOR TODO: use a negative test on "_CRT_V12_LEGACY_FUNCTIONALITY"
    // instead of a positive test on "_CRT_BEST_PRACTICES_USAGE"
...
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    _ACRTIMP onexit_t __cdecl onexit(_In_opt_ onexit_t _Func);
